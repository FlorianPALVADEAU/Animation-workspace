...
function StickyCursor({stickyElement}) {
  const { displaying, isHoveringButton, buttonDimensions } = useCursorStore()
  const [isHovered, setIsHovered] = useState(false);
  const cursor = useRef(null);
  const cursorSize = isHovered ? 90 : 25;
  ...
  const manageMouseMove = e => {
    ...
    if (isHoveringButton) {
      mouse.x.set(clientX - buttonDimensions.width /2);
      mouse.y.set(clientY - buttonDimensions.height /2);
    } else if(isHovered){
  ...
  return (
    <div className='cursorContainer'>
      <motion.div 
        transformTemplate={isHoveringButton ? cancelTemplate : template}
        style={{
          left: smoothMouse.x, 
          top: smoothMouse.y,
          scaleX: scale.x,
          scaleY: scale.y,
          display: displaying ? 'block' : 'none',
        }} 
        animate={{
          width: isHoveringButton ? buttonDimensions.width: cursorSize,
          height: isHoveringButton ? buttonDimensions.height: cursorSize,
          borderRadius: isHoveringButton ? '5px': '50%',
        }}
        className='cursor'
        ref={cursor}>
      </motion.div>
